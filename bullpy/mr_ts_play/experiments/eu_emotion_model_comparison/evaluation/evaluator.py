    ...
    def evaluate_model(
        ...
        for trial in iterator:
            ...
            try:
                ...
                # Save checkpoint after each trial (for LLM models that cost money)
                ...
                # Check for quota errors - stop evaluation immediately
                if 'quota' in error_str or 'insufficient_quota' in error_str or 'billing' in error_str:
                    logger.error(
                        f"OpenAI API quota exceeded during evaluation. "
                        f"Stopping evaluation for {model_name}. "
                        f"Completed {len(existing_predictions)}/{len(self.trials)} trials."
                    )
                    # Save what we have
                    if save_results and hasattr(model, 'get_cost_summary'):
                        with open(checkpoint_file, 'w') as f:
                            json.dump(existing_predictions, f, indent=2)
                    # Break out of loop - don't continue
                    break
