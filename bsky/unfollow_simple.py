...
# Simple approach: try to unfollow each account directly
unfollowed = 0
failed = 0
not_following = 0
...
for i, handle_to_unfollow in enumerate(handles_to_unfollow):
    try:
        ...
        # Try to find and delete the follow record
        # We'll search through follows in batches to find this specific account
        cursor = None
        follow_record = None
        
        # Search for this specific follow record
        while not follow_record:
        ...
        record_key = follow_record.uri.split('/')[-1]
