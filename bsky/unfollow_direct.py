...
# Direct unfollow approach: get follows and unfollow them immediately
unfollowed = 0
failed = 0
processed = 0
...
while unfollowed < MAX_UNFOLLOWS_PER_SESSION:
    ...
    try:
        ...
        for i, follow in enumerate(res.follows):
            ...
            try:
                # The follow object should have the URI in a different attribute
                # Let's check what attributes are available
                print(f"  Debug: follow object type: {type(follow)}")
                print(f"  Debug: follow attributes: {dir(follow)}")
                
                # Try to find the URI - it might be in a different attribute
